...
function indexing(win) {
  ...
  win.querySelector("nav").addEventListener("mouseup", () => {
    win.style.zIndex = topidx.toString();
  });
}
function dragElement(elmnt) {
  ...
  function elementDrag(e) {
    if (!max) {
      ...
      if (elmnt.offsetTop - pos2 > globInnerHeight * 0.9) {
        return;
      }
      if (elmnt.offsetLeft - pos1 > globInnerWidth * 2) {
        max = true;
        elmnt.classList.add("dvd");
      }
  ...
  if (typeof meta === "object" && meta !== null) {
    if (typeof meta.title === "string" && meta.title.trim() !== "") {
      if (typeof meta.content === "string" && meta.content.trim() !== "") {
        ...
        const contentClass =
          typeof meta.contentClass === "string" ? meta.contentClass : "";
              ...
              if (isNearRight || isNearBottom) {
                ...
                function handleMouseMove(e) {
                  if (isResizing) {
                    ...
                    if (
                      startWidth + dx >= window.innerWidth ||
                      startHeight + dy >= window.innerHeight
                    ) {
                      return;
                    }
...
let velocity = 0.0001;
setInterval(() => {
  if (document.querySelector(".dvd") == null) {
    return;
  }
