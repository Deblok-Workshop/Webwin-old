...
minesweeper.grid.prototype.generate = function (number_mines, seed) {
  ...
  this.grid_area
    .addEventListener("mouseover", function (e) {
      ...
      if (self.minesweeper.mouse.left && self.minesweeper.mouse.right) {
        // Shortcut to reveal 8 surrounding tiles
        var coordinates = self.get_coordinates_from_td(e.target);
        var surrounding_tiles = self.get_surrounding_tiles(
          coordinates.x,
          coordinates.y,
        );
...
minesweeper.grid.prototype.print_tiles = function () {
  var t = $.createElement("table").setAttribute({
    cellspacing: 0,
    cellpadding: 0,
  });
...
minesweeper.grid.prototype.all_mines_marked = function () {
  ...
  for (var y = 0; y < this.height; y++) {
    for (var x = 0; x < this.width; x++) {
      ...
      if (
        this.tiles[x][y].get_state() === "flag" &&
        !this.tiles[x][y].is_a_mine()
      ) {
        return false;
      }
