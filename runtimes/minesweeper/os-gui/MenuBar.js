((exports) => {
  ...
  function display_hotkey(text) {
    ...
    return text
      .replace(/([^&]|^)&([^&\s])/, "$1<span class='menu-hotkey'>$2</span>")
      .replace(/&&/g, "&");
  }
  ...
  function MenuBar(menus) {
    ...
    function get_direction() {
      return window.get_direction
        ...
        : getComputedStyle(menus_el).direction;
    }
    ...
    const any_open_menus = () =>
      [...document.querySelectorAll(".menu-popup")].some(
        (popup_el) => popup_el.style.display !== "none",
      );
    ...
    function send_info_event(item) {
      ...
      if (window.jQuery) {
        ...
        const extraParam = {
          toString() {
            console.warn(
              "jQuery extra parameter for info event is deprecated, use event.detail instead",
            );
            return description;
          },
        };
    ...
    function handleKeyDown(e) {
      ...
      const active_menu_popup =
        active_menu_popup_el && menu_popup_by_el.get(active_menu_popup_el);
      ...
      const parent_item_el =
        parent_item_el_by_popup_el.get(active_menu_popup_el);
      ...
      switch (e.keyCode) {
        ...
        case 39: // Right
          ...
          if (
            ...
            parent_item_el.setAttribute("aria-expanded", "false");
            send_info_event(
              active_menu_popup.parentMenuPopup.menuItems[
                active_menu_popup.parentMenuPopup.itemElements.indexOf(
                  parent_item_el,
                )
              ],
            );
        ...
        default:
          ...
          const item_els = active_menu_popup
            ...
            : top_level_menus.map(
                (top_level_menu) => top_level_menu.menu_button_el,
              );
          ...
          for (const item_el of item_els) {
            ...
            item_els_by_accelerator[accelerator_key] =
              item_els_by_accelerator[accelerator_key] || [];
            ...
            if (matching_item_els.length === 1) {
              ...
              if (menu_item_el.classList.contains("menu-button")) {
                const top_level_menu = top_level_menus.find(
                  (top_level_menu) =>
                    top_level_menu.menu_button_el === menu_item_el,
                );
      ...
      menu_items.forEach((item, item_index) => {
        ...
        if (item === MENU_DIVIDER) {
          ...
          item_el.setAttribute(
            "role",
            item.checkbox ? "menuitemcheckbox" : "menuitem",
          );
          ...
          item_el.setAttribute(
            "aria-label",
            remove_hotkey(item.label || item.item),
          );
          ...
          item_el.setAttribute(
            "aria-keyshortcuts",
            item.ariaKeyShortcuts || item.shortcut || item.shortcutLabel,
          );
          ...
          menu_popup_el.addEventListener("update", () => {
            ...
            if (item.checkbox && item.checkbox.check) {
              ...
              checkbox_area_el.querySelector("svg").style.visibility = checked
                ? "visible"
                : "hidden";
          ...
          if (item.submenu) {
            ...
            submenu_area_el.innerHTML = `
					...
					`;
            menu_popup_el.addEventListener("update", () => {
              submenu_area_el.querySelector("svg").style.transform =
                get_direction() === "rtl" ? "scaleX(-1)" : "";
            });
