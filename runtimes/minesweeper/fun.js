...
window.onkeydown = Konami.code(function () {
  ...
  if (!frameElement) {
    return console.warn("Nope, must run within iframe context");
  }

  var $mouse = parent
    .$(
      "<img src='programs/jspaint/images/cursors/default.png' class='minesweeper-playing-cursor'>",
    )
    .appendTo("body");
  ...
  iid = setInterval(function () {
    var tds = $("td");
    for (var i = 0; i < 50; i++) {
      //purposely limited number of tries
      var td = tds[~~(Math.random() * tds.length)];
      if (is_unclicked_safe_tile(td)) {
        break;
      }
    }
    for (var i = 0; i < 50; i++) {
