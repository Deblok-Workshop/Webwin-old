...
minesweeper.prototype.build = function (width, height) {
  ...
  this.grid_area = $.createElement("td").setAttribute({
    colspan: width,
    rowspan: height,
  });
  ...
  this.header_td_mine_count = $.createElement("td").style({
    width: 53,
    "text-align": "center",
  });
  this.header_td_face = $.createElement("td").style({ "text-align": "center" });
  this.header_td_timer = $.createElement("td").style({
    width: 53,
    "text-align": "center",
  });
...
minesweeper.prototype.new_game = function (width, height, number_mines) {
  ...
  this.play_table
    .removeEventListener("mousedown,mouseup")
    .style({ cursor: "default", border: "1px solid #444" });
  this.play_table
  ...
  this.mine_counter = new minesweeper.ssd(
    this.header_td_mine_count,
    number_mines,
  );
...
$.ready(function () {
  ...
  $(document).addEventListener("mousedown", function (e) {
    ...
    if (
      minesweeper_.mouse.left ||
      (minesweeper_.mouse.right && minesweeper_.mouse.left)
    ) {
      ...
      if (minesweeper_.mouse.left && minesweeper_.mouse.right) {
        ...
        var surrounding_tiles = minesweeper_.grid.get_surrounding_tiles(
          coordinates.x,
          coordinates.y,
        );
  ...
  window.addEventListener("mouseup", function (e) {
    ...
    if (
      minesweeper_.mouse.left ||
      (minesweeper_.mouse.left && minesweeper_.mouse.right)
    ) {
      var click_coordinates = minesweeper_.grid.get_coordinates_from_td(
        e.target,
      );
      ...
      if (minesweeper_.mouse.left && minesweeper_.mouse.right) {
        ...
        for (var i in minesweeper_.grid.highlighted_tiles) {
          if (minesweeper_.grid.highlighted_tiles[i].get_state() === "flag")
            flag_count++;
        }
        ...
        if (
          flag_count ==
            minesweeper_.grid.tiles[click_coordinates.x][
              click_coordinates.y
            ].get_mine_count() &&
          minesweeper_.grid.tiles[click_coordinates.x][
            click_coordinates.y
          ...
          for (var i in minesweeper_.grid.highlighted_tiles) {
            var coordinates = minesweeper_.grid.get_coordinates_from_td(
              minesweeper_.grid.highlighted_tiles[i].get_element(),
            );
