((exports) => {
  ...
  function $Window(options) {
    ...
    if (options.toolWindow) {
      ...
      function setupIframe(iframe) {
        if (!focus_update_handlers_by_container.has(iframe)) {
          ...
          setTimeout(() => {
            // for iframe src to be set? I forget.
      ...
      function make_focus_in_out_handler(logical_container_el, is_root) {
        ...
        return function handle_focus_in_out(event) {
          const container_node =
            logical_container_el.tagName == "IFRAME"
          ...
          if (!outside_or_at_exactly && newly_focused.tagName === "IFRAME") {
    ...
    Object.defineProperty($w, "task", {
      ...
      set(new_task) {
        console.warn(
          "DEPRECATED: use $w.setMinimizeTarget(taskbar_button_el) instead of setting $window.task object",
        );
        task = new_task;
      },
    });
    ...
    const warn_iframe_access = (iframe, error) => {
      const log_template = (message) => [
        `OS-GUI.js failed to access an iframe (${element_to_string(iframe)}) for focus integration.
${message}
Original error:
`,
        error,
      ...
      if (iframe.srcdoc) {
        ...
        try {
          ...
          console.error(
            ...log_template(
              `This may be a bug in OS-GUI. Is this a cross-origin iframe? Failed to parse URL (${parse_error}).`,
            ),
          );
          return;
        }
      ...
      if (cross_origin) {
        if (options.iframes?.ignoreCrossOrigin && !warned_iframes.has(iframe)) {
          console.warn(
            ...log_template(`Only same-origin iframes can work with focus integration (showing window as focused, refocusing last focused controls).
...
You can also disable this warning by passing {iframes: {ignoreCrossOrigin: true}} to $Window.`),
        ...
        console.error(
          ...log_template(
            `This may be a bug in OS-GUI, since it doesn't appear to be a cross-origin iframe.`,
          ),
        );
      }
    };

    const debug_focus_tracking = (
      document,
      container_el,
      descendant_el,
      is_root,
    ...
    const update_debug_focus_tracking = (svg) => {
      ...
      for (const rect of [descendant_rect, container_rect]) {
        ...
        rect_el.setAttribute(
          "stroke",
          rect === descendant_rect ? "#f44" : "#f44",
        );
        ...
        const text_el = document.createElementNS(
          "http://www.w3.org/2000/svg",
          "text",
        );
        ...
        text_el.textContent = element_to_string(
          rect === descendant_rect ? descendant_el : container_el,
        );
      ...
      for (const line of lines) {
        const line_el = document.createElementNS(
          "http://www.w3.org/2000/svg",
          "line",
        );
    ...
    const animate_debug_focus_tracking = () => {
      ...
      debug_animation_frame_id = requestAnimationFrame(
        animate_debug_focus_tracking,
      );
    ...
    if (options.resizable) {
      ...
      [
        ...
        function update_resize() {
          ...
          new_rect.width = Math.max(
            new_rect.width,
            options.minOuterWidth ?? 100,
          );
          new_rect.height = Math.max(
            new_rect.height,
            options.minOuterHeight ?? 0,
          );
          new_rect.width = Math.max(
            new_rect.width,
            (options.minInnerWidth ?? 0) + window_frame_width,
          );
          new_rect.height = Math.max(
            new_rect.height,
          ...
          if (x_axis === HANDLE_LEFT) {
            new_rect.x = Math.min(
              new_rect.x,
              rect.x + rect.width - new_rect.width,
            );
          }
          if (y_axis === HANDLE_TOP) {
            new_rect.y = Math.min(
              new_rect.y,
              rect.y + rect.height - new_rect.height,
            );
          }
      ...
      if (force && force !== true) {
        throw new TypeError(
          "force must be a boolean or undefined, not " +
            Object.prototype.toString.call(force),
        );
      }
      if (!force) {
        var e = $.Event("close");
