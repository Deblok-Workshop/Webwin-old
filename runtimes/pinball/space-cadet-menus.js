  ...
  function (event) {
    if (event.button === 2) {
      if (event.pointerType !== "touch") {
        ...
        click(mouse_x, mouse_y, ["mousedown"], {
          button: event.button,
...
function click(
  canvas_x,
  canvas_y,
  event_types = ["mouseenter", "mousedown", "mouseup"],
  options = {},
) {
...
async function handle_menu_item(
  top_level_menu_index,
  item_index,
  submenu_item_index,
) {
  close_menus();
  ...
  if (submenu_item_index !== undefined) {
    click(
      200 + top_level_menu_index * 50,
      35 + item_index * 16 + submenu_item_index * 16,
    );
    await sleep(delay);
  }
...
const observer = new MutationObserver(function (mutations) {
  menus_open =
    menu_bar.element.querySelectorAll(".menu-button[aria-expanded='true']")
      .length > 0;
  if (menus_open) {
    start_freeze_frame();
